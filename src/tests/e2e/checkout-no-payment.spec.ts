...
test("checkout surface has no card, Stripe, PaymentIntent or credential fields for visitors", async ({ page }) => {
  ...
  await expect(page.getByText(/PaymentIntent|Stripe|credenciais/i)).toHaveCount(0);
});
