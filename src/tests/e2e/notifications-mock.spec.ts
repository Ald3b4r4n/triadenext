...
test("client payment return cannot trigger a paid notification", async ({ page }) => {
  await page.goto("/pedidos/example/pagamento", { waitUntil: "commit" });

  await expect(page.getByText(/notificacao enviada|e-mail enviado/i)).toHaveCount(0);
  await expect(
