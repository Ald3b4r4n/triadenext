...
test("visitor sees an empty cart without a real database", async ({ page }) => {
  ...
  await expect(page.getByText("dev/fixture")).toBeVisible();
});
...
test("visitor can add a valid product in fallback mode", async ({ page }) => {
  ...
  await expect(page.getByText("R$ 159,90", { exact: true })).toBeVisible();
...
test("cart page continues to load without real database", async ({ page }) => {
  ...
  await expect(page.getByText("Frete, cupom, pagamento e pedido ficam fora desta fase.")).toBeVisible();
});
