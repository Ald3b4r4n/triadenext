...
test.describe("staging import post-import smoke", () => {
  test.skip(
    !stagingSmokeUrl,
    "STAGING_IMPORT_SMOKE_URL ausente; smoke remoto fica pendente."
  );

  test("checks the imported storefront and protected surfaces on a non-production URL", async ({
    page
  }) => {
    ...
    await expect(
      page.getByText(/DATABASE_URL|STRIPE_SECRET|secret|token/i)
    ).toHaveCount(0);
