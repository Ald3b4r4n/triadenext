...
import { expectAdminProtected, findPublishedProduct } from "./helpers";

test("production readiness smoke keeps the storefront flow safe and navigable", async ({
  page
}) => {
  ...
  await expect(
    page.getByText(
      /Reconstrucao|Placeholder funcional|DATABASE_URL|STRIPE_SECRET/i
    )
  ).toHaveCount(0);
...
test("production readiness smoke keeps protected areas blocked without real auth", async ({
  ...
  await expect(
    page.getByText(/Stripe|PaymentIntent|secret|token|DATABASE_URL/i)
  ).toHaveCount(0);
