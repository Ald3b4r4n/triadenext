    ...
    <div className="admin-table" role="table" aria-label="Cupons">
      ...
      {coupons.map((coupon) => (
        <div className="admin-table__row admin-table__row--coupons" role="row" key={coupon.id}>
          ...
          <span className="status-badge status-badge--draft" data-label="Status">{coupon.status}</span>
          <span data-label="Uso">
            {coupon.usedCount}
            {coupon.maxUses !== null ? `/${coupon.maxUses}` : ""}
          </span>
          <Link className="text-action" data-label="Ações" href={`/admin/cupons/${coupon.id}/editar`}>
            Editar
          </Link>
        </div>
      ))}
