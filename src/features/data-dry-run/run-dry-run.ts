...
export function runDataDryRun(
  options: RunDataDryRunOptions = {}
): RunDataDryRunResult {
  ...
  if (discovery.status === "pending-input") {
    const report = createPendingInputReport(
      discovery.source,
      discovery.expectedFiles
    );
    const files = options.writeReport
      ? writeReconciliationReport(report, outputDir, options.format ?? "both")
      : [];
    return { report, files };
  }
...
export function formatDryRunSummary(result: RunDataDryRunResult) {
  ...
  return [
    ...
    `Origem das divergências: dados=${report.summary.byOrigin.dados}, next=${report.summary.byOrigin.next}, mapeamento=${report.summary.byOrigin.mapeamento}, humana=${report.summary.byOrigin.humana}`,
    `Relatórios: ${files.length > 0 ? files.map((file) => file.replace(/\\/g, "/")).join(", ") : "não escritos"}`,
    "Este script não conecta banco, não executa migration, não importa dados, não faz upload e não faz deploy."
  ].join("\n");
}
