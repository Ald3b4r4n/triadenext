...
export function resolveStagingSmokeTarget(config: StagingSmokeConfig): {
  ...
  if (!config.url) {
    const issue = pendingConfigIssue({
      ...
      message:
        "STAGING_SMOKE_URL ou equivalente ausente; smoke real fica pendente."
    });
    issues.push(issue);
    return {
      target: null,
      ...
      check: createCheck({
        ...
        summary: "URL staging/preview não informada.",
        issues
      })
    };
  }
  ...
  if (
    ...
    const issue: StagingSmokeIssue = {
      ...
      category: "environment",
      message:
        "URL staging/preview deve usar http(s) e não pode conter credenciais.",
      blocksGoLive: true
    };
    issues.push(issue);
  }
  ...
  return {
    target:
      status === "passed"
        ? { kind: config.target, url, source: config.urlSource ?? "provided" }
        : null,
    issues,
    check: createCheck({
      id: "url-staging",
      ...
      summary:
        status === "passed"
          ? "URL staging/preview presente e sintaticamente segura."
          : "URL staging/preview bloqueada.",
      issues
    })
  };
}
