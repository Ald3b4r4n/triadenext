...
import { categories, coupons, productCategories, productImages, products, shippingRules } from "@/db/schema";
...
export function connectApprovedStagingDatabase(input: {
  ...
  if (input.preflight.status !== "planned") {
    throw new Error("Preflight precisa estar planned antes de abrir conexao staging.");
  }
  ...
  if (!connectionString) {
    throw new Error("Conexao staging ausente. Valor nao impresso por seguranca.");
  }
  ...
  if (!guard.allowed) {
    throw new Error("Conexao staging bloqueada por sinal de producao ou secret.");
  }

  return createDrizzleStagingImportStore(createDrizzleClient(connectionString));
}

export function createDrizzleStagingImportStore(database: StagingDrizzleClient): StagingImportStore {
  return {
    async countEntities() {
      const [categoryRows, productRows, imageRows, couponRows, shippingRows] = await Promise.all([
    ...
    async runInTransaction(callback) {
      return database.transaction(async (tx) =>
        callback(createDrizzleStagingImportStore(tx as unknown as StagingDrizzleClient))
      );
    },
...
export function createMemoryStagingImportStore(): StagingImportStore {
  ...
  return {
    ...
    async upsertCategory(category) {
      const status = state.categories.has(category.slug) ? "updated" : "inserted";
      state.categories.set(category.slug, category);
      return result("categories", category.slug, status);
    },
    async upsertProduct(product) {
      const existingBySlug = [...state.products.values()].find((candidate) => candidate.slug === product.slug && candidate.sku !== product.sku);
      if (existingBySlug) {
        return result("products", product.sku, "conflict", "Slug ja associado a outro SKU.");
      }
    ...
    async ensureProductCategory(productSku, categorySlug) {
      if (!state.products.has(productSku) || !state.categories.has(categorySlug)) {
        return result("products", `${productSku}:${categorySlug}`, "conflict", "Produto ou categoria ausente.");
      }
      return result("products", `${productSku}:${categorySlug}`, "skipped", "Relacionamento validado.");
    },
    async upsertProductImage(image) {
      if (!state.products.has(image.productSku)) {
        return result("productImages", image.productSku, "conflict", "Imagem referencia produto ausente.");
      }
    ...
    async upsertInventory(item) {
      if (!state.products.has(item.productSku)) {
        return result("inventory", item.productSku, "conflict", "Inventario referencia produto ausente.");
      }
      const status = state.inventory.has(item.productSku) ? "updated" : "inserted";
    ...
    async upsertShippingRule(rule) {
      const status = state.shippingRules.has(rule.ruleCode) ? "updated" : "inserted";
      state.shippingRules.set(rule.ruleCode, rule);
      return result("shippingRules", rule.ruleCode, status);
    },
    async resetApprovedScope(plan) {
      for (const item of plan.entities.productImages) state.productImages.delete(`${item.productSku}:${item.reference}`);
      for (const item of plan.entities.inventory) state.inventory.delete(item.productSku);
      for (const item of plan.entities.products) state.products.delete(item.sku);
      for (const item of plan.entities.categories) state.categories.delete(item.slug);
      for (const item of plan.entities.coupons) state.coupons.delete(item.code);
      for (const item of plan.entities.shippingRules) state.shippingRules.delete(item.ruleCode);
      return {
        status: "executed",
...
async function upsertCategory(database: StagingDrizzleClient, category: NormalizedCategory): Promise<EntityUpsertResult> {
  const [existing] = await database.select().from(categories).where(eq(categories.slug, category.slug)).limit(1);
  const parent = category.parentSlug ? await findCategoryBySlug(database, category.parentSlug) : null;

  if (category.parentSlug && !parent) {
    return result("categories", category.slug, "conflict", "Categoria pai ausente.");
  }
  ...
  if (existing) {
    await database.update(categories).set(row).where(eq(categories.id, existing.id));
...
async function upsertProduct(database: StagingDrizzleClient, product: NormalizedProduct): Promise<EntityUpsertResult> {
  const [existingBySku] = await database.select().from(products).where(eq(products.sku, product.sku)).limit(1);
  const [existingBySlug] = await database.select().from(products).where(eq(products.slug, product.slug)).limit(1);
  const category = await findCategoryBySlug(database, product.categorySlug);

  if (!category) {
    return result("products", product.sku, "conflict", "Categoria do produto ausente.");
  }
  ...
  if (existingBySku) {
    await database.update(products).set(row).where(eq(products.id, existingBySku.id));
...
async function ensureProductCategory(database: StagingDrizzleClient, productSku: string, categorySlug: string): Promise<EntityUpsertResult> {
  ...
  const [existing] = await database
    ...
    .where(and(eq(productCategories.productId, product.id), eq(productCategories.categoryId, category.id)))
    .limit(1);

  if (existing) {
    return result("products", `${productSku}:${categorySlug}`, "skipped", "Relacionamento ja existia.");
  }

  await database.insert(productCategories).values({ productId: product.id, categoryId: category.id });
  return result("products", `${productSku}:${categorySlug}`, "inserted", "Relacionamento criado.");
}

async function upsertProductImage(database: StagingDrizzleClient, image: NormalizedProductImage): Promise<EntityUpsertResult> {
  const product = await findProductBySku(database, image.productSku);

  if (!product) {
    return result("productImages", image.productSku, "conflict", "Produto da imagem ausente.");
  }

  if (image.isCover) {
    await database.update(productImages).set({ isCover: false }).where(eq(productImages.productId, product.id));
  }

  const [existing] = await database
    .select()
    .from(productImages)
    .where(and(eq(productImages.productId, product.id), eq(productImages.pathname, image.reference)))
    .limit(1);
  const row = {
  ...
  if (existing) {
    await database.update(productImages).set(row).where(eq(productImages.id, existing.id));
    return result("productImages", `${image.productSku}:${image.reference}`, "updated");
  }

  await database.insert(productImages).values(row);
  return result("productImages", `${image.productSku}:${image.reference}`, "inserted");
}

async function upsertInventory(database: StagingDrizzleClient, item: NormalizedInventoryItem): Promise<EntityUpsertResult> {
  const product = await findProductBySku(database, item.productSku);

  if (!product) {
    return result("inventory", item.productSku, "conflict", "Produto do inventario ausente.");
  }

  await database
    .update(products)
    .set({ stockQuantity: item.isAvailable ? item.availableQuantity : 0, updatedAt: new Date() })
    .where(eq(products.id, product.id));
  return result("inventory", item.productSku, "updated");
}

async function upsertCoupon(database: StagingDrizzleClient, coupon: NormalizedCoupon): Promise<EntityUpsertResult> {
  const [existing] = await database.select().from(coupons).where(eq(coupons.code, coupon.code)).limit(1);
  const row = {
    code: coupon.code,
    type: coupon.type,
...
async function upsertShippingRule(database: StagingDrizzleClient, rule: NormalizedShippingRule): Promise<EntityUpsertResult> {
  ...
  if (existing) {
    await database.update(shippingRules).set(row).where(eq(shippingRules.id, existing.id));
...
async function resetApprovedScope(database: StagingDrizzleClient, plan: StagingImportPlan): Promise<ResetExecutionResult> {
  const before = await createDrizzleStagingImportStore(database).countEntities();
  const approvedSkus = new Set(plan.entities.products.map((product) => product.sku));
  const approvedProducts = await database.select().from(products);

  for (const product of approvedProducts.filter((candidate) => approvedSkus.has(candidate.sku))) {
    await database.delete(productImages).where(eq(productImages.productId, product.id));
    await database.delete(productCategories).where(eq(productCategories.productId, product.id));
  ...
  const ruleCodes = new Set(plan.entities.shippingRules.map((rule) => rule.ruleCode));
  for (const rule of allShipping.filter((candidate) => ruleCodes.has(readShippingImportCode(candidate) ?? ""))) {
  ...
  return {
    ...
    message: "Reset executado apenas para chaves aprovadas em staging/dev remoto."
  };
}

async function findCategoryBySlug(database: StagingDrizzleClient, slug: string): Promise<CategoryRow | null> {
  const [row] = await database.select().from(categories).where(eq(categories.slug, slug)).limit(1);
  return row ?? null;
}

async function findProductBySku(database: StagingDrizzleClient, sku: string): Promise<ProductRow | null> {
  const [row] = await database.select().from(products).where(eq(products.sku, sku)).limit(1);
  return row ?? null;
}

async function findShippingRuleByImportCode(database: StagingDrizzleClient, ruleCode: string): Promise<ShippingRuleRow | null> {
...
function readShippingImportCode(row: ShippingRuleRow) {
  ...
  if (metadata && typeof metadata === "object" && "stagingImportRuleCode" in metadata) {
...
function summarizeCounts(status: EntityUpsertResult["status"], counts: Record<ImportEntityName, number>) {
