  ...
  return (
    <main className="page-shell">
      ...
      <div className="admin-table" role="table" aria-label="Categorias administrativas">
        <div className="admin-table__row admin-table__row--categories admin-table__row--head" role="row">
        ...
        {categories.map((category) => (
          <div className="admin-table__row admin-table__row--categories" role="row" key={category.id}>
            <span data-label="Categoria" role="cell">
              <strong>{category.name}</strong>
              <small>{category.description}</small>
            </span>
            <span data-label="Slug" role="cell">{category.slug}</span>
            <span data-label="Status" role="cell">{category.isActive ? "Ativa" : "Inativa"}</span>
          </div>
        ))}
      </div>
    </main>
