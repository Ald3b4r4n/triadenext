...
const products = [
  ["33333333-3333-4333-8333-333333333333", categories[0].id, "Dev Produto Publicado", "dev-produto-publicado", "DEV-SEED-PUB-001", "published", 15990, "159.90", 8, seedDate],
  ["44444444-4444-4444-8444-444444444444", categories[0].id, "Dev Produto Rascunho", "dev-produto-rascunho", "DEV-SEED-DRAFT-001", "draft", 9900, "99.00", 4, null],
  ["55555555-5555-4555-8555-555555555555", categories[0].id, "Dev Produto Futuro", "dev-produto-futuro", "DEV-SEED-FUTURE-001", "published", 12000, "120.00", 3, "2099-01-10T12:00:00.000Z"],
  ["66666666-6666-4666-8666-666666666666", categories[0].id, "Dev Produto Sem Estoque", "dev-produto-sem-estoque", "DEV-SEED-OOS-001", "published", 11000, "110.00", 0, seedDate],
  ["77777777-7777-4777-8777-777777777777", categories[1].id, "Dev Produto Inativo", "dev-produto-inativo", "DEV-SEED-INACTIVE-001", "inactive", 8000, "80.00", 5, seedDate]
];

if (!databaseUrl) {
  console.error("DATABASE_URL ausente. Seed bloqueado sem conectar banco real.");
  process.exit(1);
}
...
try {
  ...
  for (const product of products) {
    const [id, categoryId, name, slug, sku, status, priceCents, price, stockQuantity, publishedAt] =
      product;
    ...
    await sql`
      ...
      values (
        ...
        1, ${slug === "dev-produto-publicado"}, ${publishedAt}, ${name}, 'Seed ficticio de desenvolvimento.', ${seedDate}
      )
      on conflict (slug) do update set
        name = excluded.name,
        sku = excluded.sku,
