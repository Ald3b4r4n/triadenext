...
const forbiddenOpsFragments = ["vercel", "db:migrate", "drizzle-kit migrate", "db:seed"];
...
function run() {
  ...
  const unsafeOps = Object.entries(scripts)
    ...
    .filter(([, command]) =>
      forbiddenOpsFragments.some((fragment) => String(command).includes(fragment))
    )
    .map(([name]) => name);

  console.log("Readiness de build local");
  console.log("Este script nao chama Vercel, banco, migration real ou provider externo.");
  ...
  if (missing.length > 0) {
    console.error(`Scripts locais obrigatorios ausentes: ${missing.join(", ")}`);
    process.exitCode = 1;
  }

  if (unsafeOps.length > 0) {
    console.error(`Scripts ops com comandos proibidos: ${unsafeOps.join(", ")}`);
    process.exitCode = 1;
  }
}
